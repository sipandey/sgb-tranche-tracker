import { batch, execute, queryAll, updateSettings } from "@/lib/db";
 ...
 */
export async function seedDemoSession(sessionDate = "2026-08-07"): Promise<{
  sessionDate: string;
  metrics: number;
}> {
  const goldRate = 149020;
  ...
  const tranches = await queryAll<{
    ...
    `SELECT isin, tranche_code, maturity_date, units_per_bond
  ...
  stmts.push({
    sql: `INSERT INTO trading_calendar (session_date, nse_ok, bse_ok) VALUES (?, 1, 1)
  ...
  await runRulesForSession(sessionDate);
